...
const char * const BITCOIN_CONF_FILENAME = "dashfirst.conf";
const char * const BITCOIN_PID_FILENAME = "dashfirstd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "dashfirst" is a composite category enabling all DashFirst-related debug output
            if(ptrCategory->count(string("dashfirst"))) {
...
#else
    const char* pszModule = "dashfirst";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DashFirstCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DashFirstCore
    // Mac: ~/Library/Application Support/DashFirstCore
    // Unix: ~/.dashfirstcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DashFirstCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DashFirstCore";
#else
    // Unix
    return pathRet / ".dashfirstcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty dashfirst.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override dashfirst.conf
